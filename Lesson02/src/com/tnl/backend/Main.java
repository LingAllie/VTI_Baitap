...
//		DataInitialization.initializeData();
        ...
        GroupAccount groupAccount5 = new GroupAccount(group3, account2, new Date());
        GroupAccount groupAccount6 = new GroupAccount(group4, account2, new Date());
        GroupAccount groupAccount7 = new GroupAccount(group2, account2, new Date());
		...
		if (account2.getDepartment() == null) {
			...
			System.out.println("Phong ban cua nhan vien nay la " + account2.getDepartment().getDepartmentName());
		}
		
		
		// Question 2
		GroupAccount[] gacc = {groupAccount1, groupAccount2, groupAccount3, groupAccount4, groupAccount5, groupAccount6, groupAccount7};
		List<String> new_gacc = new ArrayList<>();

        for (int i = 0; i < gacc.length; i++) {
            if (gacc[i].getAccount().getAccountId() == account2.getAccountId()) {
                new_gacc.add(gacc[i].getGroup().getGroupName());
            }
        }
		if (new_gacc.isEmpty()) {
		...
		/*  
	        String[] new_gacc = new String[gacc.length];
	        int j = 0;
	
	        for (int i = 0; i < gacc.length; i++) {
	            if (gacc[i].getAccount().getAccountId() == account2.getAccountId()) {
	                new_gacc[j] = gacc[i].getGroup().getGroupName();
	                j++;
	            }
	        }
	        ...
	        if (j == 0) {
	            ...
	            System.out.println("Group của nhân viên này là " + new_gacc[0]);
	        } else if (j == 2) {
	            System.out.println("Group của nhân viên này là " + new_gacc[0] + ", " + new_gacc[1]);
	        } else if (j == 3) {
	            System.out.println("Nhân viên này là người quan trọng, tham gia nhiều group");
	        } else {
	            System.out.println("Nhân viên này là người hăng say, tham gia tất cả các group");
	        } 
	     */
		
		
		// Question 3
		String message = (account2.getDepartment() == null) ? 
			    "Nhân viên này chưa có phòng ban" : 
			    "Phòng ban của nhân viên này là " + account2.getDepartment().getDepartmentName();
		System.out.println(message);
			
			
		// Question 4
		String message2 = (account1.getPosition().getPositionName() == "Dev") ? 
			    "Day la developer" : 
